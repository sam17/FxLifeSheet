...
class Question:
    ...
    def from_json(cls, data):
        try:
            ...
            return cls(key, question, type, maxValue, minValue, isVisibleInVisualizer, buttons)
        except KeyError:
            raise ValueError('Invalid data structure')

    def __repr__(self):
        return f"Question(key={self.key}, question={self.question}, type={self.type}, maxValue={self.maxValue}, minValue={self.minValue}, isVisibleInVisualizer={self.isVisibleInVisualizer}, options={self.buttons})"
...
create_table_query = f"CREATE TABLE IF NOT EXISTS {table_name} (key VARCHAR(255), question VARCHAR(255), type VARCHAR(255), maxValue VARCHAR(255), minValue VARCHAR(255), isVisibleInVisualizer VARCHAR(255), buttons VARCHAR(255));"
...
for item in questions:
    ...
    cursor.execute(insert_query, (item.key, item.question, item.type,
